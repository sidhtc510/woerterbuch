...
export default function DerDasDieGame() {
    ...
    const translateHandler = async (word) => {
        const req_word = await translateViaAi(word);
                            ...
                            <>
                                ...
                                {score.list.wordsWithArtikle.length > 0 &&
                                    <div className='flex flex-col gap-2 m-4 px-10 py-2 w-fit mx-auto'>
                                        {score.list.wordsWithArtikle.map((el, index) => <p key={index} className={`border-b border-l rounded-sm py-1 px-2 ${el.isCorrect ? 'border-emerald-300 bg-emerald-100' : 'border-red-300 bg-red-100'}`} onClick={() => translateHandler(el.word)}> {el.word}</p>)}
                                    </div>
                                }
                            </>
                        )}
