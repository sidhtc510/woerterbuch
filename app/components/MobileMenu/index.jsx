    ...
    return (
        <div className='flex gap-1 md:hidden'>
            ...
            <div className={`fixed w-full left-0 top-24 h-[calc(100vh-96px)] bg-slate-200 flex flex-col items-center justify-start gap-1 z-20 ${isOpen ? 'visible opacity-100' : 'invisible opacity-0'} ease-in-out duration-500 `}>
                ...
                <Link onClick={handleLinkClick} className='w-fit flex items-center gap-1 pointer-events-auto rounded-md bg-indigo-600 px-3 py-2 text-[0.8125rem] font-semibold leading-5 text-white hover:bg-indigo-500 cursor-pointer select-none m-3' href={'/'}>Nomen</Link>
                <Link onClick={handleLinkClick} className='w-fit flex items-center gap-1 pointer-events-auto rounded-md bg-indigo-600 px-3 py-2 text-[0.8125rem] font-semibold leading-5 text-white hover:bg-indigo-500 cursor-pointer select-none m-3' href={'/verbs'}>Verbs</Link>
            </div>
        </div>
    )
}
